...
8. Der t-Wert ist `-2.982`. D.h., das beobachtete Ergebnis hat in der <i>t</i>-Verteilung mit 6,495 Freiheitsgraden (siehe Schlussabschnitt des Outputs) eine eben solche Ausprägung. Der p-Wert ist nun der relative Anteil möglicher, extremerer Werte unterhalb der Verteilung. Die Funktion `pt()` gibt für bestimmte t-Werte die Fläche für niedrigere Werte raus. Probiert einmal was ihr erhaltet, wenn ihr den ermittelten t-Wert (`q` für Quantil) und die Freiheitsgrade (`df`) in die Funktion eingebt. Wie könnte der Zusammenhang zum p-Wert im Ergebnis aussehen?
...
12. Lenke nun deine Aufmerksamkeit auf den Schlussteil. Hier siehst du eine Zusammenfassung über die Gesamtqualität des Modells. Von besonderem Interesse ist das `Multiple R-squared`. Es angibt wie viel Varianz das Modell insgesamt am Kriterium erklären kann. Was meinst du, ist der Wert gross oder klein?
